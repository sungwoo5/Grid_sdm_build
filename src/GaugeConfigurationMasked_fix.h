// [032924 sungwoo]
// Substitute SU3 objects as generalized SUNc objects:
//   SU3 -> FundamentalMatrices,
//   SU3Adjoint -> AdjointMatrices, 
//   8 -> AdjDim=FundamentalMatrices::AdjointDimension,
// to bypass the compilation error, but not sure if it will actually work..
/*!
...
private:
  ...
  typedef typename AdjointMatrices::AMatrix AdjMatrix;
  typedef typename AdjointMatrices::LatticeAdjMatrix  AdjMatrixField;
  typedef typename AdjointMatrices::LatticeAdjVector  AdjVectorField;

  int AdjDim=FundamentalMatrices::AdjointDimension; // sungwoo
  ...
  void InsertForce(GaugeField &Fdet,AdjVectorField &Fdet_nu,int nu)
  {
    ...
    Fdet_pol=Zero();
    for(int e=0;e<AdjDim;e++){
      ColourMatrix te;
      FundamentalMatrices::generator(e, te);
  ...
  {
    std::cout << " sungwoo: Compute_MpInvJx_dNxxdSy "<<std::endl;
    ...
    const int Ngen = AdjointMatrices::Dimension;
    Complex ci(0,1);
    ...
    for(int a=0;a<Ngen;a++) {
      tta-=usecond();
      FundamentalMatrices::generator(a, ta);
      ta = 2.0 * ci * ta;
      // Qlat Tb = 2i Tb^Grid
	...
	FundamentalMatrices::generator(c, tc);
	tc = 2.0*ci*tc;
	tp-=usecond(); 
	D = Ta( tc *UtaU); // 2ms
#if 1
	FundamentalMatrices::LieAlgebraProject(Dbc_opt,D,c); // 5.5ms
#else
	for(int b=0;b<Ngen;b++){
	  FundamentalMatrices::generator(b, tb);
	  tmp =-trace(ci*tb*D); 
  ...
  {
    std::cout << " sungwoo: ComputeNxy "<<std::endl;
...
#if 1
      FundamentalMatrices::LieAlgebraProject(NxAd,Nx,b);
#else
      for(int c=0;c<Ngen;c++) {
	FundamentalMatrices::generator(c, tc);
...
public:
  ...
  {
    std::cout << " sungwoo: logDetJacobianForceLevel "<<std::endl;
    ...
    ZxAd = Zero();
    for(int b=0;b<AdjDim;b++) {
      // Adj group sets traceless antihermitian T's -- Guido, really????
      FundamentalMatrices::generator(b, tb);         // Fund group sets traceless hermitian T's
      AdjointMatrices::generator(b,TRb);
      TRb=-TRb;
    ...
    time=-usecond();
#if 1
    std::vector<AdjMatrixField>  dJdX;    dJdX.resize(AdjDim,grid);
    std::vector<AdjMatrix> TRb_s; TRb_s.resize(AdjDim);
    ...
    for(int b=0;b<AdjDim;b++){
      AdjointMatrices::generator(b, TRb_s[b]);
      dJdX[b] = TRb_s[b];
    }
    aunit = ComplexD(1.0);
    ...
    for(int b=0;b<AdjDim;b++){
      aunit = ComplexD(1.0);
      AdjointMatrices::generator(b, TRb); //dt2
  ...
  {
    std::cout << " sungwoo: logDetJacobianLevel "<<std::endl;
...
#if 0
      FundamentalMatrices::LieAlgebraProject(Ncb,tmp,b);
#else
      for(int c=0;c<Ngen;c++) {
	FundamentalMatrices::generator(c, Tc);
