...
if [ -d ./build_grid_omp_unified_Nc${Nc} ];
then
  rm -rf ./build_grid_omp_unified_Nc${Nc}
fi

mkdir  ./build_grid_omp_unified_Nc${Nc}
cd ./build_grid_omp_unified_Nc${Nc}
...
${SRCDIR}/Grid/configure \
	 ...
	 --prefix=$INSTALLDIR/Grid_omp_unified_Nc${Nc} \
	 CXX=$CC \
	 MPICXX=$CC \
